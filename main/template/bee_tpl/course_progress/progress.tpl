                    ...
                    <table width="100%" class="table table-hover">
                        ...
                        <tr>
                            ...
                            <td>
                                {% if tutor %}
                                    <div class="pull-right">
                                        <a title="{{ 'EditThematicPlan'|get_lang }}"
                                           href="index.php?{{ _p.web_cid_query }}&origin=thematic_details&action=thematic_plan_list&thematic_id={{ item.id }}&width=700&height=500'"
                                    ...
                                    {% if item.thematic_plan is empty %}
                                        <div class="alert-thematic">
                                            <div class="alert alert-info"
                                                 role="alert">{{ 'StillDoNotHaveAThematicPlan'|get_lang }}</div>
                                        </div>
                                    {% else %}
                            ...
                            <td>
                                {% if tutor %}
                                    <div class="pull-right">
                                        <a title="{{ 'NewThematicAdvance'|get_lang }}"
                                           href="index.php?{{ _p.web_cid_query }}&action=thematic_advance_add&thematic_id={{ item.id }}"
                                           class="btn btn-default">
                                ...
                                <div class="thematic-advance">
                                    <table width="100%" class="table">
                                        ...
                                        <tr>
                                            <td style="width: 90%" class="thematic_advance_content"
                                                id="thematic_advance_content_id_{{ advance.id }}">
                                                ...
                                                {% if tutor %}
                                                    <div class="toolbar-actions">
                                                        <div id="thematic_advance_tools_{{ advance.id }}"
                                                             class="thematic_advance_actions">
                                                            <div class="btn-group btn-group-sm">
                                                                <a class="btn btn-default btn-sm"
                                                                   href="index.php?{{ _p.web_cid_query }}&action=thematic_advance_edit&thematic_id={{ item.id }}&thematic_advance_id={{ advance.id }}"
                                                                   title="{{ 'Edit' |get_lang }}">
                                                                    <i class="fa fa-pencil" aria-hidden="true"></i>
                                                                </a>
                                                                <a class="btn btn-default btn-sm"
                                                                   onclick="javascript:if(!confirm('{{ 'AreYouSureToDelete'|get_lang }}')) return false;"
                                                                   href="index.php?{{ _p.web_cid_query }}&action=thematic_advance_delete&thematic_id={{ item.id }}&thematic_advance_id={{ advance.id }}"
                            ...
                            <td style="width: 10%; {{ color }}" id="td_done_thematic_{{ advance.id }}">
                                ...
                                {% endif %}
                                <center>
                                    <input type="radio" class="done_thematic" id="done_thematic_{{ advance.id }}"
                                           name="done_thematic" value="{{ advance.id }}" {{ check }}
                                           onclick="update_done_thematic_advance(this.value)">
                                </center>
                            {% else %}
                                </td>
                            {% endif %}
