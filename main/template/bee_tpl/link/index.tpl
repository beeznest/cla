...
<div class="btn-toolbar actions-bar">
    {% if is_allowed_to_edit %}
        <div class="btn-group edit">
            ...
            <a href="javascript:void(0)" onclick="delete_all();return false;" class="btn btn-default"
               title="{{ 'DeleteAll'|get_lang }}">
    ...
    {% endif %}
    <div class="btn-group">
        <a href="javascript:void(0)" onclick="expand_all();return false;" class="btn btn-default"
           title="{{ 'ShowAll'|get_lang }}">
            <em class="size-32 icon-expand"></em>
        </a>
        <a href="javascript:void(0)" onclick="collapse_all();return false;" class="btn btn-default"
           title="{{ 'ShowNone'|get_lang }}">
...
<div class="data">
    <ul style="list-style: none; margin-left:0;" class="links sortable">
        {% for link in links %}
            ...
            <li id="link_{{ link.id }}" class="link {{ link_class }}" data-id="{{ link.id }}"
                data-c_id="{{ link.c_id }}" data-type="link">
                <a class="icon" target="{{ link.target }}"
                   href="{{ root }}&amp;action=go&amp;id={{ link.id }}&amp;c_id={{ link.c_id }}">
                    {{ 'link.png' |icon(22) }}
                </a>
                <a class="title" target="{{ link.target }}"
                   href="{{ root }}&amp;action=go&amp;id={{ link.id }}&amp;c_id={{ link.c_id }}">
                ...
                {% if is_allowed_to_edit %}
                    <div style="float:right;">
                        ...
                        <a href="javascript:void(0)" onclick="delete_link('link_{{ link.id }}', this);return false;"
                           title="{{ 'Delete'|get_lang }}" class="">
    ...
    <ul id="link_categories" class="categories sortable" style="list-style: none; margin-left:0;">
        {% for category in categories %}
            <li id="category_{{ category.id }}" class="link_category collapsible" data-id="{{ category.id }}"
                data-c_id="{{ category.c_id }}" data-type="category">
                        ...
                        {% for link in category.links %}
                            ...
                            <li id="link_{{ link.id }}" class="link {{ link_class }}" data-id="{{ link.id }}"
                                ...
                                {% if is_allowed_to_edit %}
                                    <div style="float:right;">
                                        <a href="javascript:void(0)"
                                           onclick="validate_link('link_{{ link.id }}', this);return false;"
                                           title="{{ 'CheckURL'|get_lang }}"
